...
async function buildAction(projectDir, options) {
  ...
  if (!options.type) {
    options.type = _buildTools().BuildType.Generic;
  } // it will accept all types of builds but will fail later on unsupported types.


  if (!Object.values(_buildTools().BuildType).includes(options.type)) {
...
function _default(program) {
  program.command('build [project-dir]').description('Build an app binary for your project, signed and ready for submission to the Google Play Store / App Store.').option('-p --platform <platform>', 'Platform: [android|ios]', /^(android|ios)$/i).option('-t --type <type>', 'Type: [generic|managed|]', /^(generic|managed)$/i).asyncActionProjectDir(buildAction);
