...
async function constructBundleQueryParamsAsync(projectRoot, opts) {
  ...
  let {
    exp
  } = await (0, _config().readConfigJsonAsync)(projectRoot); // SDK11 to SDK32 require us to inject hashAssetFiles through the params, but this is not
  // needed with SDK33+
...
async function constructWebAppUrlAsync(projectRoot) {
  ...
  const host = hostType === 'localhost' ? 'localhost' : _ip().default.address();
  let urlType = 'http';
