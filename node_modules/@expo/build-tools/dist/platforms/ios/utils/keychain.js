...
const spawn_1 = __importDefault(require("../../../utils/spawn"));
const fastlane_1 = __importDefault(require("./fastlane"));
class Keychain {
    constructor(logger) {
        this.created = false;
        this.logger = logger;
    ...
    async cleanUpKeychains() {
        const { stdout } = await spawn_1.default('security', ['list-keychains'], { stdio: 'pipe' });
        const keychainList = (stdout.match(/"(.*)"/g) || []).map((i) => i.slice(1, i.length - 1));
    ...
    async findIdentitiesByTeamId(teamId) {
        const { output } = await spawn_1.default('security', ['find-identity', '-v', '-s', `(${teamId})`, this.keychainPath], { stdio: 'pipe' });
        return output.join('');
    }
}
